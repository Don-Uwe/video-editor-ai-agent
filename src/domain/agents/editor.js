...
function resolveShot(shotId, index) {
    ...
    const match = index.shots.find((shot) => shot.source_file === sourceFile &&
...
export async function runEditor(plan, footageIndexPath, outputDir) {
    const index = FootageIndexSchema.parse(JSON.parse(readFileSync(footageIndexPath, "utf8")));
...
export async function refinePlan(plan, footageIndexPath) {
    void footageIndexPath;
    return plan;
}
export async function runReviewer(brief, videoPath) {
